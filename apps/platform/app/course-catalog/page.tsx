...
export default function CourseCatalogPage() {
  ...
  return (
    ...
    >
      ...
      <Link href="/explore" style={{ color: '#6B9B6B', fontSize: 13, display: 'inline-block', marginTop: 12 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {sorted.map((path) => {
              ...
              return (
                <tr key={path.slug} style={{ borderBottom: '1px solid #1A1A1E' }}>
                  <td style={{ padding: '12px 8px', color: '#6B6B70' }}>{path.launch_rank ?? '—'}</td>
                  <td style={{ padding: '12px 8px', color: '#E8E8EC' }}>{path.name}</td>
                  <td style={{ padding: '12px 8px', color: '#8A8A8E', maxWidth: 220 }}>{path.outcome}</td>
                  <td style={{ padding: '12px 8px' }}>
                    <span style={{ color: c.text, fontSize: 11 }}>
                      {path.status} · {EXPLORE_STATUS_LABELS[path.status]}
                    </span>
                  </td>
                  <td style={{ padding: '12px 8px', color: '#8A8A8E' }}>{path.tier}</td>
                  <td style={{ padding: '12px 8px', color: '#6B6B70', fontSize: 11 }}>{path.section_id}</td>
                  <td style={{ padding: '12px 8px' }}>
                    <Link href={href} style={{ color: '#6B9B6B', fontSize: 12 }}>
                      {href}
                    </Link>
                  </td>
                </tr>
