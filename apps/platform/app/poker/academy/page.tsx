...
import { POKER_ACADEMY_LEVELS } from '../../../lib/poker-world';

export default function PokerAcademyPage() {
  return (
    <section style={{ marginTop: 16 }}>
      <h1 style={{ fontWeight: 300, fontSize: '2rem', margin: 0 }}>Academy</h1>
      {POKER_ACADEMY_LEVELS.map((level) => (
        <article key={level.slug} style={{ padding: 24, marginTop: 12, background: '#111114', borderRadius: 8 }}>
          ...
          {level.level === 1 && (
            <Link href="/poker/missions/track-bankroll" style={{ display: 'inline-block', marginTop: 16, color: '#6B8BB8', fontSize: 14 }}>Start Mission 1 →</Link>
          )}
        </article>
      ))}
    </section>
