    ...
    <section style={{ marginTop: 16 }}>
      ...
      <div style={{ marginTop: 28 }}>
        {AI_BUILDER_PLAYGROUND_LABS.map((lab) => (
          <article key={lab.slug} style={{ padding: 24, marginBottom: 16, background: 'var(--foundry-surface)', borderRadius: 8, border: '1px solid var(--foundry-border-subtle)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
              <h2 style={{ fontSize: 16, fontWeight: 400, margin: 0, color: 'var(--foundry-text)' }}>{lab.title}</h2>
              <span style={{ fontSize: 11, color: 'var(--foundry-text-faint)' }}>{lab.unlockLevel}+</span>
            </div>
            <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 12 }}>{lab.description}</p>
            <h3 style={{ fontSize: 12, color: 'var(--foundry-success)', margin: '20px 0 8px', fontWeight: 400 }}>Try this</h3>
            <ul style={{ margin: 0, paddingLeft: 20, color: 'var(--foundry-text-muted)', fontSize: 13, lineHeight: 1.9 }}>
      ...
      <Link href="/ai-builder/portfolio" style={{ color: 'var(--foundry-success)', fontSize: 14 }}>
