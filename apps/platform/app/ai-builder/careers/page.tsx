    ...
    <section style={{ marginTop: 16 }}>
      ...
      <div style={{ marginTop: 28 }}>
        {AI_BUILDER_CAREERS.map((career) => (
          <article key={career.title} style={{ padding: 20, marginBottom: 10, background: 'var(--foundry-surface)', borderRadius: 8 }}>
            <h2 style={{ fontSize: 15, fontWeight: 400, color: 'var(--foundry-text)', margin: 0 }}>{career.title}</h2>
            <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>{career.connection}</p>
          </article>
        ))}
      </div>
      ...
      <Link href="/future-proof" style={{ color: 'var(--foundry-success)', fontSize: 14 }}>
