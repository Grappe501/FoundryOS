...
export default function TransformationPage() {
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#08080A', color: '#E8E8EC', padding: '2rem', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <p style={{ color: '#C8A96E', fontSize: 15, marginTop: 8 }}>{PASS_009_FACTORY_MISSION}</p>
      <p style={{ color: '#8A8A8E', fontSize: 14, marginTop: 12 }}>{PASS_009_GUIDANCE}</p>
      <Link href="/transformation-graph" style={{ color: '#C8A96E', fontSize: 13, display: 'inline-block', marginTop: 12 }}>
      ...
      <section style={{ marginTop: 28, padding: 20, background: '#0F0F12', border: '1px solid #2A2520', borderRadius: 8 }}>
        <h2 style={{ fontSize: 14, color: '#C8A96E', margin: 0 }}>North Star</h2>
      ...
      <section style={{ marginTop: 32 }}>
        <h2 style={{ fontSize: 14, color: '#C8A96E' }}>Object Hierarchy</h2>
        ...
        <div style={{ marginTop: 16, padding: 16, background: '#0F0F12', borderRadius: 8, fontSize: 12 }}>
          {Object.entries(COMMUNITY_SPEAKER_GRAPH_EXAMPLE).map(([k, v]) => (
            <div key={k} style={{ padding: '3px 0', color: '#8A8A8E' }}>
              <span style={{ color: '#C8A96E' }}>{k.replace(/_/g, ' ')}:</span> {v}
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ marginTop: 32 }}>
        <h2 style={{ fontSize: 14, color: '#C8A96E' }}>Transformation Templates</h2>
      ...
      <section style={{ marginTop: 32 }}>
        <h2 style={{ fontSize: 14, color: '#C8A96E' }}>DNA Blueprints ({DNA_REGISTRY.length})</h2>
      ...
      <section style={{ marginTop: 32 }}>
        <h2 style={{ fontSize: 14, color: '#C8A96E' }}>Academic Opportunity</h2>
      ...
      <section style={{ marginTop: 32 }}>
        <h2 style={{ fontSize: 14, color: '#C8A96E' }}>Business Model</h2>
        {FOUNDRY_PRODUCTS.map((p) => (
          <div key={p.slug} style={{ padding: '12px 0', borderBottom: '1px solid #1A1A1E', fontSize: 13 }}>
            ...
            <span style={{ color: '#C8A96E', marginLeft: 8 }}>{p.price}</span>
