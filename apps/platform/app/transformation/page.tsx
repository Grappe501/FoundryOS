...
export default function TransformationPage() {
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--foundry-bg)', color: 'var(--foundry-text)', padding: '2rem', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12 }}>{PASS_009_GUIDANCE}</p>
      ...
      <section style={{ marginTop: 28, padding: 20, background: 'var(--foundry-surface)', border: '1px solid var(--foundry-border-warm)', borderRadius: 8 }}>
        ...
        <p style={{ color: 'var(--foundry-text)', fontSize: 16, marginTop: 12 }}>{PRIMARY_DASHBOARD_QUESTION}</p>
      ...
      <section style={{ marginTop: 32 }}>
        ...
        <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 8 }}>{HIERARCHY_FLOW}</p>
        {FOUNDRY_OBJECT_HIERARCHY.map((h) => (
          <div key={h.key} style={{ padding: '8px 0', fontSize: 13, color: 'var(--foundry-text-muted)', borderBottom: '1px solid var(--foundry-border-subtle)' }}>
            <span style={{ color: 'var(--foundry-text)' }}>{h.label}</span>
            <span style={{ color: 'var(--foundry-text-dim)', marginLeft: 8 }}>— {h.question}</span>
          </div>
        ))}
        <div style={{ marginTop: 16, padding: 16, background: 'var(--foundry-surface)', borderRadius: 8, fontSize: 12 }}>
          {Object.entries(COMMUNITY_SPEAKER_GRAPH_EXAMPLE).map(([k, v]) => (
            <div key={k} style={{ padding: '3px 0', color: 'var(--foundry-text-muted)' }}>
      ...
      <section style={{ marginTop: 32 }}>
        ...
        {TRANSFORMATION_TEMPLATES.map((t) => (
          <div key={t.slug} style={{ padding: '12px 0', borderBottom: '1px solid var(--foundry-border-subtle)' }}>
            <div style={{ color: 'var(--foundry-text)', fontSize: 14 }}>{t.display_name}</div>
            <div style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 4 }}>{t.layers.join(' · ')}</div>
          </div>
        ))}
      </section>
      ...
      <section style={{ marginTop: 32 }}>
        ...
        {[PUBLIC_SPEAKING_DNA, AI_BUILDER_DNA].map((dna) => (
          <pre key={dna.domain} style={{ marginTop: 12, padding: 16, background: 'var(--foundry-surface)', borderRadius: 8, fontSize: 11, color: 'var(--foundry-text-muted)', overflow: 'auto' }}>
      ...
      <section style={{ marginTop: 32 }}>
        ...
        <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12 }}>Not: {ACADEMIC_OPPORTUNITY.not.join(' · ')}</p>
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>{ACADEMIC_OPPORTUNITY.but.join(' · ')}</p>
        <p style={{ color: 'var(--foundry-text-dim)', fontSize: 11, marginTop: 8 }}>{ACADEMIC_OPPORTUNITY.principle}</p>
      </section>

      <section style={{ marginTop: 32 }}>
        <h2 style={{ fontSize: 14, color: 'var(--foundry-primary)' }}>Business Model</h2>
        {FOUNDRY_PRODUCTS.map((p) => (
          <div key={p.slug} style={{ padding: '12px 0', borderBottom: '1px solid var(--foundry-border-subtle)', fontSize: 13 }}>
            <span style={{ color: 'var(--foundry-text)' }}>{p.name}</span>
            <span style={{ color: 'var(--foundry-primary)', marginLeft: 8 }}>{p.price}</span>
            <div style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 4 }}>{p.includes.join(' · ')}</div>
          </div>
        ))}
      </section>
    </main>
