...
function Stat({ label, value, sub }: { label: string; value: string | number; sub?: string }) {
  return (
    <div style={{ padding: 16, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
      <div style={{ fontSize: 24, fontWeight: 300, color: 'var(--foundry-text)' }}>{value}</div>
      <div style={{ fontSize: 11, color: 'var(--foundry-text-faint)', marginTop: 4 }}>{label}</div>
      {sub && <div style={{ fontSize: 10, color: 'var(--foundry-text-dim)', marginTop: 4 }}>{sub}</div>}
    </div>
  );
}
...
function Rate({ label, value }: { label: string; value: number }) {
  return (
    <div style={{ padding: 12, background: 'var(--foundry-surface)', borderRadius: 6 }}>
...
export default async function OperatorRevenuePage() {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--foundry-bg)', color: 'var(--foundry-text)', padding: '2rem', maxWidth: 1040, margin: '0 auto' }}>
      ...
      <Link href="/operator/business" style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginLeft: 16 }}>Business dashboard</Link>
      <Link href="/operator/analytics" style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginLeft: 16 }}>Analytics</Link>
      <Link href="/operator/revenue/verify" style={{ color: 'var(--foundry-success)', fontSize: 13, marginLeft: 16 }}>PASS-029A verify →</Link>
      ...
      {!snapshot ? (
        ...
        <>
          ...
          {consistency && (
            <section style={{ marginTop: 24, padding: 20, background: 'var(--foundry-surface)', borderRadius: 8, border: `1px solid ${consistency.aligned ? 'var(--foundry-success-bg)' : 'var(--foundry-primary-border-dim)'}` }}>
          ...
          <section style={{ marginTop: 32, padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
            <h2 style={{ fontSize: 14, color: 'var(--foundry-success)', margin: 0 }}>By tier</h2>
            <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, fontSize: 13 }}>
              <div>
                <p style={{ color: 'var(--foundry-text-faint)', margin: 0 }}>Clicks</p>
                {Object.entries(snapshot.by_tier_click).map(([t, n]) => (
                  <p key={t} style={{ color: 'var(--foundry-text)', margin: '4px 0' }}>{t}: {n}</p>
                ))}
              </div>
              <div>
                <p style={{ color: 'var(--foundry-text-faint)', margin: 0 }}>Initiated</p>
                {Object.entries(snapshot.by_tier_initiated).map(([t, n]) => (
                  <p key={t} style={{ color: 'var(--foundry-text)', margin: '4px 0' }}>{t}: {n}</p>
                ))}
              </div>
              <div>
                <p style={{ color: 'var(--foundry-text-faint)', margin: 0 }}>Paid</p>
          ...
          <section style={{ marginTop: 32 }}>
            ...
            <div style={{ marginTop: 12 }}>
              {snapshot.by_world.map((w) => (
                <div key={w.world_slug} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--foundry-border-subtle)', fontSize: 13 }}>
                  <Link href={`/community/${w.world_slug}`} style={{ color: 'var(--foundry-text)' }}>{w.world_slug}</Link>
          ...
          <section style={{ marginTop: 32 }}>
            ...
            <div style={{ marginTop: 12 }}>
              {snapshot.by_mission.length === 0 ? (
                <p style={{ color: 'var(--foundry-text-faint)', fontSize: 13 }}>No mission conversion data yet.</p>
              ) : (
                snapshot.by_mission.map((m) => (
                  <div key={`${m.world_slug}:${m.mission_slug}`} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--foundry-border-subtle)', fontSize: 13 }}>
                    <span style={{ color: 'var(--foundry-text)' }}>{m.world_slug} / {m.mission_slug}</span>
                    <span style={{ color: 'var(--foundry-text-muted)' }}>{m.completions} done · {m.upgrade_initiated} initiated · {m.upgrade_completed} paid</span>
                  </div>
                ))
              )}
            </div>
          ...
          <section style={{ marginTop: 32, padding: 20, background: 'var(--foundry-surface)', borderRadius: 8 }}>
            <h2 style={{ fontSize: 14, color: 'var(--foundry-text-faint)', margin: 0 }}>Recent revenue events</h2>
            <div style={{ marginTop: 12, maxHeight: 320, overflow: 'auto' }}>
              {snapshot.recent_upgrade_events.map((e) => (
                <div key={e.id} style={{ fontSize: 12, padding: '6px 0', borderBottom: '1px solid var(--foundry-border-subtle)', color: 'var(--foundry-text-muted)' }}>
            ...
            <p style={{ color: 'var(--foundry-text-dim)', fontSize: 11, marginTop: 10 }}>{snapshot.event_count.toLocaleString()} events analyzed</p>
          </section>
        </>
      )}
    </main>
