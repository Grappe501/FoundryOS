...
export default async function OperatorLearningPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--foundry-bg)', color: 'var(--foundry-text)', padding: '2rem', maxWidth: 960, margin: '0 auto' }}>
      ...
      <Link href="/operator/revenue/verify" style={{ color: 'var(--foundry-success)', fontSize: 13, marginLeft: 16 }}>029A verify →</Link>

      <p style={{ color: 'var(--foundry-success)', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', marginTop: 16 }}>PASS-034 (deferred)</p>
      ...
      <section style={{ marginTop: 24, padding: 20, background: '#0F1210', border: '1px solid #2A3A2A', borderRadius: 8 }}>
        <h2 style={{ fontSize: 14, color: 'var(--foundry-success)', margin: 0 }}>Locked rules</h2>
      ...
      <section style={{ marginTop: 28 }}>
        ...
        <div style={{ marginTop: 16, display: 'grid', gap: 10 }}>
          {LEARNING_LANE_METRICS.map((m) => (
            <div key={m.key} style={{ padding: 14, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
              <div style={{ fontSize: 14, color: 'var(--foundry-text)' }}>{m.label}</div>
              <div style={{ fontSize: 12, color: 'var(--foundry-text-faint)', marginTop: 4 }}>{m.description}</div>
            </div>
          ))}
        </div>
      ...
      {biz && (
        <section style={{ marginTop: 28, padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
          ...
          <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, fontSize: 13 }}>
            <div><span style={{ color: 'var(--foundry-text-faint)' }}>Return signal</span><br />{biz.active_testers} active testers</div>
            <div><span style={{ color: 'var(--foundry-text-faint)' }}>Upgrade intent</span><br />{biz.upgrade_intent}</div>
            <div><span style={{ color: 'var(--foundry-text-faint)' }}>Paid</span><br />{biz.upgrade_completed} conversions · ${biz.mrr_usd} MRR</div>
          </div>
        </section>
      )}

      <section style={{ marginTop: 28 }}>
        <h2 style={{ fontSize: 14, color: 'var(--foundry-text-faint)', margin: 0 }}>Starting worlds</h2>
        <div style={{ marginTop: 12, fontSize: 13, color: 'var(--foundry-text-muted)' }}>
          {LEARNING_LANE_WORLDS.map((w) => (
            <p key={w.slug} style={{ margin: '6px 0' }}>
              <Link href={`/${w.slug}`} style={{ color: 'var(--foundry-text)' }}>{w.label}</Link>
              <span style={{ color: 'var(--foundry-text-faint)' }}> — {w.priority}</span>
            </p>
          ))}
        </div>
      </section>
      ...
      <section style={{ marginTop: 28, padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        ...
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>Operators must not assign adult worlds to student accounts.</p>
      ...
      <section style={{ marginTop: 32, padding: 20, background: '#1A1410', border: '1px solid #3A3020', borderRadius: 8 }}>
        <p style={{ color: 'var(--foundry-primary)', fontSize: 13, margin: 0 }}>
          Ready after PASS-032: {getMissionCount('ai-builder')}+ missions per Life Leverage world. Use <Link href="/operator/invites" style={{ color: 'var(--foundry-text)' }}>/operator/invites</Link> when immersion gate clears.
        </p>
      </section>
    </main>
  );
