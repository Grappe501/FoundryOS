...
export default function ChessWorldPage() {
  return (
    <section style={{ marginTop: 16 }}>
      <section style={{ padding: 32, background: 'var(--foundry-surface)', border: '1px solid #3A4A3A', borderRadius: 8 }}>
        ...
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 16, marginTop: 16, lineHeight: 1.7, maxWidth: 640 }}>{CHESS_CORE_PROMISE}</p>
        ...
        <div style={{ marginTop: 28, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <Link
            ...
            style={{ padding: '14px 24px', background: '#3A4A3A', borderRadius: 6, color: 'var(--foundry-text)', fontSize: 14, textDecoration: 'none' }}
          >
            Start Mission 1 →
          </Link>
          <Link
            href="/chess/academy"
            style={{ padding: '14px 24px', border: '1px solid var(--foundry-border-subtle)', borderRadius: 6, color: 'var(--foundry-text-muted)', fontSize: 14, textDecoration: 'none' }}
          >
            Academy levels
          </Link>
          <Link
      ...
      <section style={{ marginTop: 24, padding: 24, background: 'var(--foundry-surface)', borderRadius: 8 }}>
        ...
        {CHESS_IMMERSION.tracks.map((t) => (
          <p key={t.slug} style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 12 }}>
            <strong style={{ color: 'var(--foundry-text)', fontWeight: 400 }}>{t.label}</strong> — {t.description}
          </p>
        ))}
      </section>
      ...
      <section style={{ marginTop: 24, padding: 24, background: 'var(--foundry-surface)', borderRadius: 8 }}>
        ...
        {CHESS_MISSIONS.map((m) => (
          <Link
            ...
            style={{ display: 'block', padding: '14px 0', borderBottom: '1px solid var(--foundry-border-subtle)', textDecoration: 'none', color: 'var(--foundry-text)', fontSize: 14 }}
          >
      ...
      <section style={{ marginTop: 24, padding: 24, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        ...
        {CHESS_ACADEMY_LEVELS.map((l) => (
          <p key={l.slug} style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 12 }}>
            <strong style={{ color: 'var(--foundry-text)', fontWeight: 400 }}>
      ...
      <section style={{ marginTop: 24, padding: 24, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        ...
        {CHESS_COMMUNITY.features.map((f) => (
          <p key={f.title} style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 12 }}>
            <strong style={{ color: 'var(--foundry-text)', fontWeight: 400 }}>{f.title}</strong> — {f.description}
          </p>
        ))}
      </section>

      <p style={{ marginTop: 24, fontSize: 12, color: 'var(--foundry-text-dim)' }}>
        <Link href="/explore/chess" style={{ color: 'var(--foundry-text-faint)' }}>
          Explore path
        </Link>
      </p>
    </section>
