...
export default async function BourbonAtlasTermPage({ params }: Props) {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--foundry-bg)', color: 'var(--foundry-text)', padding: '2rem', maxWidth: 720, margin: '0 auto' }}>
      ...
      <LinkedParagraph segments={enrichAtlasParagraph(entry.shortDefinition, term)} style={{ color: 'var(--foundry-text)', fontSize: 17, marginTop: 16, fontWeight: 400 }} />
      ...
      {entry.examples.length > 0 && (
        <section style={{ marginTop: 24, padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
          ...
          <ul style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12, paddingLeft: 18, lineHeight: 1.8 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            {bottleLinks.map((b) => (
              <Link
                key={b.slug}
                href={b.href}
                style={{ padding: '8px 14px', background: 'var(--foundry-surface-raised)', borderRadius: 999, color: 'var(--foundry-text)', fontSize: 13, textDecoration: 'none', border: '1px solid var(--foundry-border)' }}
              >
                {b.label} →
              </Link>
            ))}
      ...
      {entry.relatedTerms.length > 0 && (
        <section style={{ marginTop: 28 }}>
          <h2 style={{ fontSize: 14, color: 'var(--foundry-success)', fontWeight: 400 }}>Related terms</h2>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
      ...
      {entry.cousinIdeas.length > 0 && (
        <section style={{ marginTop: 20 }}>
          <h2 style={{ fontSize: 14, color: 'var(--foundry-text-faint)', fontWeight: 400 }}>Cousin ideas</h2>
          <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>{entry.cousinIdeas.join(' · ')}</p>
        </section>
      )}
      ...
      {entry.forwardLinks.length > 0 && (
        <section style={{ marginTop: 28, padding: 20, background: '#0F1210', borderRadius: 8, border: '1px solid #2A3A2A' }}>
          ...
          <ul style={{ margin: '14px 0 0', paddingLeft: 18, color: 'var(--foundry-text-muted)', fontSize: 13, lineHeight: 1.9 }}>
            {entry.forwardLinks.map((l) => (
              <li key={l.href + l.label}>
                <Link href={l.href} style={{ color: 'var(--foundry-success)' }}>{l.label}</Link>
              </li>
            ))}
          </ul>
        </section>
...
function AtlasSection({ title, body, accent, term }: { title: string; body: string; accent?: boolean; term: string }) {
  return (
    <section style={{ marginTop: 24, maxWidth: 720 }}>
      <h2 style={{ fontSize: 13, color: accent ? 'var(--foundry-primary)' : 'var(--foundry-text-faint)', fontWeight: 400, letterSpacing: '0.06em', textTransform: 'uppercase', margin: 0 }}>
        {title}
      </h2>
...
function RabbitHoleSection({ title, links }: { title: string; links: { href: string; label: string }[] }) {
  ...
  return (
    <section style={{ marginTop: 20 }}>
      <h2 style={{ fontSize: 13, color: 'var(--foundry-text-faint)', fontWeight: 400 }}>{title}</h2>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
        {links.map((l) => (
          <Link key={l.href} href={l.href} style={{ padding: '6px 12px', background: 'var(--foundry-surface-raised)', borderRadius: 999, color: 'var(--foundry-text-muted)', fontSize: 12, textDecoration: 'none' }}>
            {l.label}
          </Link>
        ))}
      </div>
