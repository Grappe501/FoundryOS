...
export default function BourbonAtlasIndexPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--foundry-bg)', color: 'var(--foundry-text)', padding: '2rem', maxWidth: 820, margin: '0 auto' }}>
      <Link href="/bourbon" style={{ color: 'var(--foundry-text-faint)', fontSize: 13 }}>← Bourbon world</Link>
      <Link href="/bourbon/level-1" style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginLeft: 16 }}>Level 1 HQ</Link>
      <Link href="/bourbon/glossary" style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginLeft: 16 }}>Quick glossary</Link>
      ...
      <section style={{ marginTop: 32 }}>
        <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, margin: 0 }}>{entries.length} entries</p>
        <div style={{ marginTop: 16, display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
          {entries.map((e) => (
            ...
            >
              ...
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 8, lineHeight: 1.5 }}>{e.shortDefinition}</p>
            </Link>
          ))}
        </div>
      </section>
