...
export default function BourbonVerticalPage() {
  return (
    <main style={{ minHeight: '100vh', backgroundColor: 'var(--foundry-bg)', color: 'var(--foundry-text)', padding: '2rem', maxWidth: 900, margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>Appreciate Craft · Become a Bourbon Steward</p>
      <p style={{ color: 'var(--foundry-success)', fontSize: 12, marginTop: 8 }}>
        Consumer world:{' '}
        <Link href="/bourbon" style={{ color: 'var(--foundry-success)' }}>/bourbon</Link>
      </p>

      <section style={{ marginTop: 32, padding: 24, background: 'var(--foundry-surface)', borderRadius: 8, border: '1px solid var(--foundry-primary-border-dim)' }}>
      ...
      <section style={{ marginTop: 24, padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        ...
        <p style={{ color: 'var(--foundry-text)', fontSize: 14, marginTop: 12, lineHeight: 1.7 }}>Learn to appreciate craft, host tastings, and build a bourbon journey that lasts decades.</p>
      </section>
    </main>
  );
}
