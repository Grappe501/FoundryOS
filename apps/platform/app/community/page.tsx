...
export default async function CommunityProofPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#08080A', color: '#E8E8EC', padding: '2rem', maxWidth: 900, margin: '0 auto' }}>
      ...
      <p style={{ color: '#6B6B70', fontSize: 13, marginTop: 8 }}>{PASS_012_COMMUNITY_EXIT}</p>
      ...
      <section style={{ marginTop: 24, padding: 20, background: '#0F0F12', border: '1px solid #2A2520', borderRadius: 8 }}>
        ...
        <p style={{ color: '#8A8A8E', fontSize: 13, marginTop: 12 }}>{community.tagline}</p>
      ...
      <section style={{ marginTop: 24, padding: 20, background: '#0F0F12', borderRadius: 8 }}>
        ...
        {COMMUNITY_EXAMPLES.slice(1).map((ex) => (
          <div key={ex.slug} style={{ padding: '10px 0', borderBottom: '1px solid #1A1A1E', fontSize: 13 }}>
            ...
            <span style={{ color: '#6B6B70', marginLeft: 8 }}>({ex.community_type})</span>
          </div>
        ))}
      </section>
      ...
      <section style={{ marginTop: 32, fontSize: 12, color: '#4A4A4E' }}>
        <p>
          Collections: <Link href="/collections" style={{ color: '#6B6B70' }}>/collections</Link>
          {' · '}
          Evidence: <Link href="/evidence" style={{ color: '#6B6B70' }}>/evidence</Link>
          {' · '}
          Loop: <Link href="/loop" style={{ color: '#6B6B70' }}>/loop</Link>
        </p>
