  ...
  return (
    <section style={{ marginTop: 16 }}>
      ...
      <section style={{ marginTop: 28, padding: 24, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        ...
        {c.memberRoles.map((role) => (
          <div key={role.role} style={{ marginTop: 16 }}>
            <p style={{ color: 'var(--foundry-text)', fontSize: 14, margin: 0 }}>{role.role}</p>
            <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 6, lineHeight: 1.6 }}>{role.description}</p>
          </div>
        ))}
      </section>
      ...
      <section style={{ marginTop: 16, padding: 24, background: 'var(--foundry-surface)', borderRadius: 8 }}>
        ...
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12, lineHeight: 1.7 }}>{c.weeklyChallenge}</p>
      </section>

      <section style={{ marginTop: 16, padding: 24, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        <h2 style={{ fontSize: 14, color: 'var(--foundry-text)', margin: 0 }}>Showcase format</h2>
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12, lineHeight: 1.7 }}>{c.showcaseFormat}</p>
      </section>

      <section style={{ marginTop: 16, padding: 24, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
        <h2 style={{ fontSize: 14, color: 'var(--foundry-text)', margin: 0 }}>Peer feedback loop</h2>
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12, lineHeight: 1.7 }}>{c.peerFeedbackLoop}</p>
      </section>

      <section style={{ marginTop: 16, padding: 24, background: 'var(--foundry-surface)', borderRadius: 8, border: `1px solid ${bundle.accentColor}33` }}>
        <h2 style={{ fontSize: 14, color: bundle.accentColor, margin: 0 }}>Mentor role</h2>
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12, lineHeight: 1.7 }}>{c.mentorRole}</p>
      </section>
    </section>
  );
}
