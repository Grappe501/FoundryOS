...
export function WorldPortfolioDepth({
  ...
  return (
    <div>
        ...
        {entries.length === 0 ? (
          ...
          entries.map((e) => (
            <div key={e.missionSlug} style={{ padding: 16, marginTop: 12, background: 'var(--foundry-surface)', borderRadius: 8 }}>
              ...
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 8 }}>{new Date(e.completedAt).toLocaleDateString()}</p>
              {e.reflection && <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{e.reflection}</p>}
            </div>
          ))
        )}
      </section>
      ...
      {nextMission && (
        <section style={{ marginTop: 24, padding: 20, background: 'var(--foundry-surface)', borderRadius: 8, border: `1px solid ${bundle.accentColor}33` }}>
          <p style={{ color: 'var(--foundry-text-faint)', fontSize: 11, margin: 0, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Next recommended action</p>
