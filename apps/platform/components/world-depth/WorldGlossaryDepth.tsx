    ...
    <section style={{ marginTop: 16 }}>
      ...
      <dl style={{ marginTop: 28 }}>
        {bundle.glossary.map((item) => (
          <div key={item.term} style={{ padding: '20px 0', borderBottom: '1px solid #1A1A1E' }}>
            <dt style={{ color: bundle.accentColor, fontSize: 15, fontWeight: 400 }}>{item.term}</dt>
            <dd style={{ color: '#E8E8EC', fontSize: 13, margin: '8px 0 0', lineHeight: 1.6 }}>{item.definition}</dd>
            <dd style={{ color: '#8A8A8E', fontSize: 12, margin: '8px 0 0', lineHeight: 1.6 }}>
              <strong style={{ color: '#6B6B70', fontWeight: 400 }}>Why it matters:</strong> {item.whyItMatters}
            </dd>
            <dd style={{ color: '#6B6B70', fontSize: 12, margin: '8px 0 0', lineHeight: 1.6 }}>
              <strong style={{ fontWeight: 400 }}>Example:</strong> {item.example}
            </dd>
            {item.relatedTerms.length > 0 && (
              <dd style={{ color: '#4A4A4E', fontSize: 11, margin: '8px 0 0' }}>
