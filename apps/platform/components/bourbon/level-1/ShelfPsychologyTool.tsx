    ...
    <div>
      ...
      <div style={{ marginTop: 20, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {BOURBON_BOTTLES.map((b) => (
          <button key={b.slug} type="button" onClick={() => toggle(b.slug)} style={{ padding: '8px 12px', fontSize: 11, borderRadius: 6, border: `1px solid ${selected.includes(b.slug) ? ACCENT : '#2A2A2E'}`, background: selected.includes(b.slug) ? '#2A2520' : 'transparent', color: selected.includes(b.slug) ? '#E8E8EC' : '#8A8A8E', cursor: 'pointer' }}>
            {b.name}
          </button>
        ))}
      </div>
      ...
      {insight && selected.length > 0 && (
        <article style={{ marginTop: 28, padding: 24, background: '#111114', borderRadius: 12, border: `1px solid ${ACCENT}44` }}>
          ...
          <p style={{ color: '#E8E8EC', fontSize: 18, marginTop: 12, lineHeight: 1.5 }}>{insight.headline}</p>
          <p style={{ color: '#8A8A8E', fontSize: 14, marginTop: 16 }}>You appear to like:</p>
          <ul style={{ marginTop: 8, paddingLeft: 20, color: '#E8E8EC', lineHeight: 1.8 }}>
            {insight.traits.map((t) => (
              <li key={t}>{t}</li>
            ))}
          </ul>
          <p style={{ color: '#6B6B70', fontSize: 12, marginTop: 20, fontStyle: 'italic' }}>Share: &ldquo;{insight.shareText}&rdquo;</p>
        </article>
      )}
