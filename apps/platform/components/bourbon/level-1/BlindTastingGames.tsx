...
function MysteryGame() {
  ...
  return (
    <GameBlock title="Mystery Bottle Challenge" subtitle="Color, nose, finish — guess proof, mashbill, or age">
        ...
        {c.options.map((o, i) => (
          <button
            ...
            style={{
              padding: 12,
              textAlign: 'left',
              background: picked === null ? '#0F0F12' : o.correct ? '#2A4020' : picked === i ? '#402020' : '#0F0F12',
      ...
      {picked !== null && (
        <div style={{ marginTop: 16, padding: 16, background: '#111114', borderRadius: 8 }}>
          <p style={{ color: ACCENT, fontSize: 14, margin: 0 }}>{c.reveal.name}</p>
          <p style={{ color: '#8A8A8E', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{c.reveal.teach}</p>
          <button type="button" onClick={next} style={{ marginTop: 12, padding: '8px 16px', background: '#4A4020', border: 'none', borderRadius: 6, color: '#E8E8EC', cursor: 'pointer' }}>
            Next mystery →
          </button>
        </div>
      )}
...
function DistilleryMatchGame() {
  ...
  return (
    <GameBlock title="Distillery Match" subtitle="Match flavor clues to the house">
      <p style={{ color: '#E8E8EC', fontSize: 14, lineHeight: 1.65 }}>{q.description}</p>
      <div style={{ marginTop: 16, display: 'grid', gap: 8 }}>
        {q.options.map((o) => (
          <button
            key={o.producerSlug}
            ...
            style={{
              padding: 12,
              background: picked === null ? '#0F0F12' : o.correct ? '#2A4020' : picked === o.producerSlug ? '#402020' : '#0F0F12',
              border: '1px solid #2A2A2E',
              borderRadius: 6,
              color: '#E8E8EC',
              cursor: picked === null ? 'pointer' : 'default',
      ...
      {picked && (
        <div style={{ marginTop: 16 }}>
          <p style={{ color: '#8A8A8E', fontSize: 13, lineHeight: 1.6 }}>{q.explain}</p>
          ...
          <button
            ...
            style={{ display: 'block', marginTop: 12, padding: '8px 16px', background: '#4A4020', border: 'none', borderRadius: 6, color: '#E8E8EC', cursor: 'pointer' }}
          >
            Next match →
          </button>
        </div>
  ...
  return (
    <GameBlock title="Bourbon Brackets" subtitle="Which would you buy? Your picks feed Bourbon DNA">
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, alignItems: 'center', marginTop: 8 }}>
        ...
        <span style={{ color: '#6B6B70', fontSize: 13 }}>vs</span>
...
function BottleCard({ bottle, onPick }: { bottle: { name: string; priceUsd: number; proof: number; oneLiner: string }; onPick: () => void }) {
  return (
    <button
      type="button"
      onClick={onPick}
      style={{ padding: 16, background: '#0F0F12', border: '1px solid #2A2520', borderRadius: 8, cursor: 'pointer', textAlign: 'left', color: 'inherit' }}
    >
      <p style={{ color: '#E8E8EC', fontSize: 14, margin: 0 }}>{bottle.name}</p>
      <p style={{ color: '#6B6B70', fontSize: 11, marginTop: 4 }}>${bottle.priceUsd} · {bottle.proof} pf</p>
      <p style={{ color: '#8A8A8E', fontSize: 12, marginTop: 8, lineHeight: 1.4 }}>{bottle.oneLiner}</p>
    </button>
  );
}
...
function GameBlock({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return (
    <article style={{ marginTop: 32, padding: 24, background: '#111114', borderRadius: 12, border: '1px solid #1A1A1E' }}>
      <h2 style={{ fontSize: 17, fontWeight: 400, margin: 0, color: '#E8E8EC' }}>{title}</h2>
