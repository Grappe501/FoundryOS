      ...
      <div style={{ marginTop: 24, display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
        {bottles.map((b) => {
          ...
          return (
            ...
            >
              <p style={{ color: 'var(--foundry-text)', fontSize: 15, margin: 0 }}>{bottle.name}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 6 }}>${bottle.priceUsd} · {bottle.proof} proof</p>
...
export function BottleProgressionView({ slug }: { slug: string }) {
  ...
  if (!b || !bottle) return <p style={{ color: 'var(--foundry-text-muted)' }}>Bottle not found.</p>;

  return (
    <div>
      <Link href="/bourbon/bottles" style={{ color: 'var(--foundry-text-faint)', fontSize: 13 }}>← All bottles</Link>
      <Link href={`/bourbon/producers/${bottle.producerSlug}`} style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginLeft: 12 }}>
        {bottle.producerName} →
      </Link>
      ...
      <LinkedParagraph segments={enrichBottleIntro(slug)} style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 10 }} />
      <p style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginTop: 8 }}>${bottle.priceUsd} · {bottle.proof} proof · {bottle.mashbill}</p>
      ...
      {depth && (
        <>
          <section style={{ marginTop: 28 }}>
            ...
            <p style={{ color: 'var(--foundry-text-muted)', fontSize: 15, marginTop: 12, lineHeight: 1.85 }}>{depth.history}</p>
          </section>
          <section style={{ marginTop: 24 }}>
            <h3 style={{ fontSize: 14, color: 'var(--foundry-text)', fontWeight: 400 }}>How it&apos;s made</h3>
            <p style={{ color: 'var(--foundry-text-muted)', fontSize: 15, marginTop: 12, lineHeight: 1.85 }}>{depth.howItsMade}</p>
          </section>
          <section style={{ marginTop: 24 }}>
            <h3 style={{ fontSize: 13, color: 'var(--foundry-text-faint)' }}>Distinguishing facts</h3>
          ...
          {depth.masterNote && (
            <blockquote style={{ marginTop: 24, padding: 16, borderLeft: `3px solid ${ACCENT}`, background: 'var(--foundry-surface)', color: '#A8A8AC', fontSize: 14, fontStyle: 'italic' }}>
              {depth.masterNote}
            </blockquote>
          )}
          <section style={{ marginTop: 28, padding: 20, background: '#0F1018', borderRadius: 10 }}>
            ...
            <div style={{ marginTop: 16, display: 'grid', gap: 12 }}>
              {depth.pairings.map((pair) => (
                <div key={pair.occasion} style={{ padding: 14, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
                  <p style={{ color: 'var(--foundry-text)', fontSize: 14, margin: 0 }}>{pair.occasion}</p>
                  <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 6 }}>{pair.why}</p>
          ...
          {depth.compareWith.length > 0 && (
            <section style={{ marginTop: 24 }}>
              <h3 style={{ fontSize: 13, color: 'var(--foundry-text-faint)' }}>Compare side-by-side</h3>
                ...
                {depth.compareWith.map((other) => {
                  ...
                  return (
                    <Link
                      ...
                      style={{ padding: '8px 12px', background: '#1A2530', borderRadius: 999, color: 'var(--foundry-text)', fontSize: 12, textDecoration: 'none', border: `1px solid ${ACCENT}44` }}
                    >
                      vs {ob?.name ?? other} →
                    </Link>
                  );
      ...
      <section style={{ marginTop: 28 }}>
        <h3 style={{ fontSize: 13, color: 'var(--foundry-text-faint)', fontWeight: 400 }}>What you&apos;ll learn</h3>
        <ul style={{ color: 'var(--foundry-text)', fontSize: 14, lineHeight: 1.8, marginTop: 12 }}>
      ...
      <section style={{ marginTop: 24, display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
        <article style={{ padding: 16, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
          <p style={{ color: 'var(--foundry-text-faint)', fontSize: 11, margin: 0 }}>Who it&apos;s for</p>
          <p style={{ color: 'var(--foundry-text)', fontSize: 14, marginTop: 8 }}>{b.whoItsFor}</p>
        </article>
        <article style={{ padding: 16, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
          <p style={{ color: 'var(--foundry-text-faint)', fontSize: 11, margin: 0 }}>When to buy</p>
          <p style={{ color: 'var(--foundry-text)', fontSize: 14, marginTop: 8 }}>{b.whenToBuy}</p>
        </article>
      </section>

      {(b.nextBottle || b.thenBottle) && (
        <section style={{ marginTop: 28 }}>
          <h3 style={{ fontSize: 13, color: 'var(--foundry-text-faint)', fontWeight: 400 }}>Progression path</h3>
          {b.nextBottle && (
            <Link href={`/bourbon/bottles/${b.nextBottle.slug}`} style={{ display: 'block', marginTop: 12, padding: 16, background: 'var(--foundry-border-warm)', borderRadius: 8, textDecoration: 'none', border: `1px solid ${ACCENT}44` }}>
              <p style={{ color: ACCENT, fontSize: 11, margin: 0 }}>Next bottle</p>
              <p style={{ color: 'var(--foundry-text)', fontSize: 15, marginTop: 6 }}>{b.nextBottle.name}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 6 }}>{b.nextBottle.why}</p>
            </Link>
          )}
          {b.thenBottle && (
            <Link href={`/bourbon/bottles/${b.thenBottle.slug}`} style={{ display: 'block', marginTop: 10, padding: 16, background: 'var(--foundry-surface-raised)', borderRadius: 8, textDecoration: 'none' }}>
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 11, margin: 0 }}>Then</p>
              <p style={{ color: 'var(--foundry-text)', fontSize: 15, marginTop: 6 }}>{b.thenBottle.name}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 6 }}>{b.thenBottle.why}</p>
            </Link>
          )}
        </section>
      )}
