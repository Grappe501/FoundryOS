...
export function DidYouKnowStrip({ compact = false }: { compact?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--foundry-text)', fontSize: compact ? 14 : 16, marginTop: 10, lineHeight: 1.45, fontWeight: 400 }}>
        {card.headline}
      </p>
      {!compact && (
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 10, lineHeight: 1.65 }}>{card.body}</p>
      )}
      {card.rabbitHoleHref && (
