    ...
    <div>
      ...
      <div style={{ marginTop: 24, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
        {CIVIC_ENGAGEMENT_PORTFOLIO_SECTIONS.map((section) => (
          <div key={section.slug} style={{ padding: 16, background: 'var(--foundry-surface-raised)', borderRadius: 8, border: '1px solid var(--foundry-success-bg)' }}>
            ...
            <p style={{ color: 'var(--foundry-success)', fontSize: 20, fontWeight: 300, marginTop: 12 }}>{section.slug === 'ballot-research' ? entries.length : '—'}</p>
          </div>
        ))}
      </div>
      {entries.length === 0 ? (
        <p style={{ color: 'var(--foundry-text-faint)', fontSize: 14, marginTop: 24 }}>
          No missions yet.{' '}
          <Link href="/civic-engagement/missions/research-ballot" style={{ color: 'var(--foundry-success)' }}>Start Mission 1 →</Link>
        </p>
      ) : (
        entries.map((e) => (
