...
const TYPE_ACCENT: Record<string, string> = {
  ...
  spotlight: '#6B9BC9',
  debate: '#9B8FD4',
  challenge: 'var(--foundry-success)',
  rabbit_hole: '#7BA3C9',
  legendary_object: '#E8B86D',
  history: 'var(--foundry-success)',
};
...
export function WorldEventsToday({ worldSlug, accent = 'var(--foundry-success)', compact = false }: Props) {
  ...
  if (!snapshot) {
    return <p style={{ color: 'var(--foundry-text-muted)' }}>World events coming soon for this world.</p>;
  }
  ...
  return (
    <div>
      <header>
        ...
        <p style={{ color: 'var(--foundry-text-muted)', fontSize: compact ? 14 : 16, marginTop: compact ? 8 : 14, lineHeight: 1.65 }}>
          {snapshot.hero_hook}
        </p>
        <p style={{ color: 'var(--foundry-text-dim)', fontSize: 12, marginTop: 8 }}>
      ...
      {collections.length > 0 && (
        <section style={{ marginTop: 28, padding: 18, background: 'var(--foundry-surface)', borderRadius: 8, border: '1px solid var(--foundry-border-subtle)' }}>
          <p style={{ color: accent, fontSize: 11, letterSpacing: '0.06em', textTransform: 'uppercase', margin: 0 }}>
            Related collections
          </p>
          <ul style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 10, paddingLeft: 18, lineHeight: 1.8 }}>
      ...
      {snapshot.events.find((e) => e.related_community_prompt) && (
        <section style={{ marginTop: 20, padding: 18, background: 'var(--foundry-surface-raised)', borderRadius: 8, border: `1px solid ${accent}33` }}>
          <p style={{ color: 'var(--foundry-text-faint)', fontSize: 11, margin: 0 }}>Community prompt</p>
          <p style={{ color: 'var(--foundry-text)', fontSize: 14, marginTop: 8 }}>
...
function EventCard({
  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--foundry-text)', fontSize: 17, margin: '8px 0 0', fontWeight: 400 }}>{event.title}</p>
      <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 10, lineHeight: 1.55 }}>{event.short_hook}</p>
      {!interaction.viewed && (
        <p style={{ color: 'var(--foundry-text-dim)', fontSize: 12, marginTop: 8 }}>{event.why_it_matters.slice(0, 120)}…</p>
      )}
      ...
      {event.event_type === 'rabbit_hole' && (
        <button
          ...
          style={{
            marginTop: 12,
            padding: '8px 14px',
            background: interaction.saved ? 'var(--foundry-border-warm)' : 'transparent',
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        {options.map((o) => (
          <button
            ...
            style={{
              padding: '8px 14px',
              background: interaction.vote === o.id ? 'var(--foundry-border-warm)' : 'var(--foundry-surface)',
              border: `1px solid ${interaction.vote === o.id ? accent : 'var(--foundry-border)'}`,
              borderRadius: 6,
    ...
    <div style={{ marginTop: 14, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
      {options.map((o) => (
        <button
          ...
          style={{
            padding: '8px 14px',
            background: interaction.debate_choice === o.id ? 'var(--foundry-border-warm)' : 'var(--foundry-surface)',
            border: `1px solid ${interaction.debate_choice === o.id ? accent : 'var(--foundry-border)'}`,
            borderRadius: 6,
            color: 'var(--foundry-text)',
            fontSize: 12,
            cursor: 'pointer',
  ...
  return (
    <button
      ...
      style={{
        marginTop: 12,
        padding: '10px 16px',
        background: interaction.completed ? 'var(--foundry-success-bg)' : 'var(--foundry-border-warm)',
        border: 'none',
        borderRadius: 6,
        color: 'var(--foundry-text)',
        fontSize: 13,
...
export function WorldEventsTeaser({ worldSlug, accent = 'var(--foundry-success)' }: Props) {
  const snapshot = useMemo(() => getActiveWorldEvents(worldSlug), [worldSlug]);
  const [mounted, setMounted] = useState(false);
  ...
  return (
    ...
    >
      ...
      <p style={{ color: 'var(--foundry-text)', fontSize: 18, marginTop: 10, lineHeight: 1.4 }}>{snapshot.hero_title}</p>
      <p style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginTop: 8 }}>{snapshot.hero_hook}</p>
      {rivalry && <p style={{ color: '#D4847A', fontSize: 12, marginTop: 10 }}>Rivalry: {rivalry.title}</p>}
      {challenge && <p style={{ color: 'var(--foundry-success)', fontSize: 12, marginTop: 6 }}>Challenge: {challenge.title}</p>}
...
export function CrossWorldEventsPanel() {
  ...
  return (
    <section style={{ marginTop: 28 }}>
      ...
      <p style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginTop: 6 }}>Events are state — not articles. Something is happening.</p>
      <div style={{ marginTop: 14, display: 'grid', gap: 10 }}>
        {highlights.map((h) => (
          ...
          >
            <p style={{ color: 'var(--foundry-text-faint)', fontSize: 10, margin: 0, textTransform: 'uppercase' }}>{h.world_slug.replace(/-/g, ' ')}</p>
            <p style={{ color: 'var(--foundry-text)', fontSize: 15, marginTop: 6 }}>{h.hero_title}</p>
            {h.challenge_title && (
              <p style={{ color: 'var(--foundry-success)', fontSize: 12, marginTop: 6 }}>Challenge: {h.challenge_title}</p>
            )}
          </Link>
        ))}
      </div>
...
export function CommunityEventsStrip({ worldSlug, accent = 'var(--foundry-success)' }: Props) {
  const snapshot = useMemo(() => getActiveWorldEvents(worldSlug), [worldSlug]);
  ...
  return (
    <section style={{ marginTop: 24, padding: 18, background: 'var(--foundry-surface)', borderRadius: 8, border: `1px solid ${accent}33` }}>
      <p style={{ color: accent, fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', margin: 0 }}>
        This week in {worldSlug.replace(/-/g, ' ')}
      </p>
      {challenge && <p style={{ color: 'var(--foundry-text)', fontSize: 14, marginTop: 10 }}>Challenge: {challenge.title}</p>}
      {debate && <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>Debate: {debate.title}</p>}
