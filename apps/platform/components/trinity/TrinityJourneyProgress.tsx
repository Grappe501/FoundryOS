...
export function TrinityJourneyProgress({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <section style={{ marginTop: compact ? 0 : 24, padding: 24, background: 'var(--foundry-surface)', borderRadius: 8, border: '1px solid var(--foundry-border-subtle)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
        <h2 style={{ fontSize: compact ? 14 : 16, fontWeight: 400, margin: 0, color: 'var(--foundry-text)' }}>
          Your Future-Proof Journey
        </h2>
        {!compact && (
          <Link href="/my-journey" style={{ color: 'var(--foundry-success)', fontSize: 13, textDecoration: 'none' }}>
            Full dashboard →
          </Link>
        )}
      </div>
      {!mounted ? (
        <p style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginTop: 16 }}>Loading your progress…</p>
      ) : (
        <>
              ...
              return (
                <div key={world.slug}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6 }}>
                    <span style={{ color: 'var(--foundry-text)' }}>{world.name}</span>
                    <span style={{ color: world.accent }}>
                      {done}/{world.missionCount} missions
                    </span>
                  </div>
                  <div style={{ height: 6, background: 'var(--foundry-border-subtle)', borderRadius: 3, overflow: 'hidden' }}>
                    <div
                      style={{
                        width: `${pct}%`,
                        height: '100%',
