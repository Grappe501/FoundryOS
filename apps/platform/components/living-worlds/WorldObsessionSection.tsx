...
export function WorldObsessionSection({ worldSlug, accent = 'var(--foundry-success)' }: Props) {
  ...
  return (
    <section
      style={{
        marginTop: 24,
        padding: 28,
        background: 'linear-gradient(135deg, var(--foundry-surface) 0%, var(--foundry-surface-raised) 100%)',
    ...
    >
      ...
      <div style={{ marginTop: 20, display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
        {obsession.reasons.map((r) => (
          <article key={r.title}>
            <p style={{ color: 'var(--foundry-text)', fontSize: 15, margin: 0 }}>{r.title}</p>
            <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{r.body}</p>
          </article>
        ))}
      </div>
    </section>
