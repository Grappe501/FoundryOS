  ...
  return (
    <section
      style={{
        marginTop: 24,
        padding: 28,
        background: 'linear-gradient(160deg, #0A0A0E 0%, #12121A 50%, var(--foundry-surface) 100%)',
    ...
    >
      ...
      <h2 style={{ fontSize: 22, fontWeight: 400, marginTop: 12, color: 'var(--foundry-text)', lineHeight: 1.35 }}>
        {feed.headline}
      </h2>
      <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 8 }}>{feed.date_key}</p>
      ...
      <div style={{ marginTop: 24, display: 'grid', gap: 12 }}>
        {feed.items.map((item) => (
          ...
          >
            ...
            <p style={{ color: 'var(--foundry-text)', fontSize: 16, marginTop: 10, lineHeight: 1.45 }}>{item.body}</p>
            {item.href && (
      ...
      <div style={{ marginTop: 24, display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        ...
        <Link href={`/${worldSlug}/lore`} style={{ color: 'var(--foundry-text-faint)', fontSize: 14 }}>
          Enter mythology →
        </Link>
        {worldSlug === 'bourbon' && (
          <Link href="/bourbon/universe" style={{ color: 'var(--foundry-text-faint)', fontSize: 14 }}>
            Universe map →
          </Link>
        )}
      </div>
...
export function WorldTodayFull({ worldSlug, accent = 'var(--foundry-primary)' }: Props) {
  const feed = getLivingMediaFeed(worldSlug);
  if (!feed) {
    return <p style={{ color: 'var(--foundry-text-muted)' }}>Living media for this world is coming soon.</p>;
  }
  ...
  return (
    <div>
      <header>
        ...
        <p style={{ color: 'var(--foundry-text-faint)', fontSize: 13, marginTop: 8 }}>{feed.date_key}</p>
      </header>
      <div style={{ marginTop: 28, display: 'grid', gap: 12 }}>
        {feed.items.map((item) => (
          <article key={item.id} style={{ padding: 18, background: 'var(--foundry-surface-raised)', borderRadius: 10, border: `1px solid ${accent}33` }}>
