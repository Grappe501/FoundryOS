  ...
  return (
    <div>
      ...
      <header>
        ...
        {backHref && (
          <Link href={backHref} style={{ display: 'inline-block', marginTop: 16, color: 'var(--foundry-text-faint)', fontSize: 13 }}>
            ← {backLabel ?? 'Back'}
          </Link>
        )}
...
function LoreSectionBlock({
  ...
  return (
    <section id={`lore-${section}`} style={{ marginTop: 48, scrollMarginTop: 24 }}>
      ...
      <p style={{ fontSize: 12, color: 'var(--foundry-text-faint)', marginTop: 6, marginBottom: 0 }}>
      ...
      {section === 'legends' && lore.legends && (
        <div style={{ marginTop: 20, display: 'grid', gap: 12 }}>
          {lore.legends.map((leg) => (
            <Link key={leg.id} href={`/${worldSlug}/legends/${leg.id}`} style={{ display: 'block', padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 10, textDecoration: 'none', border: '1px solid var(--foundry-border-subtle)' }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 16, margin: 0 }}>{leg.title}</p>
      ...
      {section === 'debates' && lore.debates && (
        <div style={{ marginTop: 20, display: 'grid', gap: 16 }}>
          {lore.debates.map((d) => (
            <article key={d.id} style={{ padding: 22, background: 'var(--foundry-surface)', borderRadius: 12, border: '1px solid var(--foundry-border-subtle)' }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 17, margin: 0 }}>{d.title}</p>
              <div style={{ marginTop: 16, display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                <div style={{ padding: 14, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
                  ...
                  <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{d.campA.argument}</p>
                </div>
                <div style={{ padding: 14, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
                  <p style={{ color: accent, fontSize: 11, margin: 0 }}>{d.campB.label}</p>
                  <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{d.campB.argument}</p>
                </div>
              </div>
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 14, fontStyle: 'italic' }}>{d.whyPeopleReturn}</p>
            </article>
          ))}
        </div>
      )}
      ...
      {section === 'legendary-objects' && lore.legendaryObjects && (
        <div style={{ marginTop: 20, display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
          {lore.legendaryObjects.map((o) => (
            <Link key={o.id} href={`/${worldSlug}/objects/${o.id}`} style={{ display: 'block', padding: 18, background: 'var(--foundry-surface-raised)', borderRadius: 10, textDecoration: 'none', border: `1px solid ${accent}22` }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 15, margin: 0 }}>{o.name}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>{o.tagline}</p>
            </Link>
          ))}
        </div>
      )}
      ...
      {section === 'heroes' && (
        <div style={{ marginTop: 20, display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
          {lore.heroes.map((h) => (
            <article key={h.id} style={{ padding: 22, background: 'var(--foundry-surface-raised)', borderRadius: 12, border: '1px solid var(--foundry-border-subtle)' }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 18, margin: 0, fontWeight: 400 }}>{h.name}</p>
      ...
      {section === 'rivalries' && (
        <div style={{ marginTop: 20, display: 'grid', gap: 16 }}>
          {lore.rivalries.map((r) => (
            <article key={r.id} style={{ padding: 22, background: 'var(--foundry-surface)', borderRadius: 12, border: '1px solid var(--foundry-border-subtle)' }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 17, margin: 0 }}>{r.title}</p>
              <div style={{ marginTop: 16, display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                <div style={{ padding: 14, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
                  <p style={{ color: accent, fontSize: 11, margin: 0 }}>{r.sideA.label}</p>
                  <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{r.sideA.argument}</p>
                </div>
                <div style={{ padding: 14, background: 'var(--foundry-surface-raised)', borderRadius: 8 }}>
                  <p style={{ color: accent, fontSize: 11, margin: 0 }}>{r.sideB.label}</p>
                  <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{r.sideB.argument}</p>
                </div>
              </div>
              <p style={{ color: 'var(--foundry-text)', fontSize: 14, marginTop: 16, lineHeight: 1.6 }}>
                <span style={{ color: 'var(--foundry-text-faint)' }}>Foundry take: </span>
                {r.foundryTake}
              </p>
              {r.href && (
      ...
      {section === 'pilgrimages' && (
        <div style={{ marginTop: 20, display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))' }}>
          {lore.pilgrimages.map((p) => (
            <article key={p.id} style={{ padding: 18, background: 'var(--foundry-surface-raised)', borderRadius: 10, border: `1px solid ${accent}22` }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 15, margin: 0 }}>{p.title}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>{p.description}</p>
              {p.href && (
      ...
      {section === 'controversies' && (
        <div style={{ marginTop: 20, display: 'grid', gap: 16 }}>
          {lore.controversies.map((c) => (
            <article key={c.id} style={{ padding: 22, background: 'var(--foundry-surface-raised)', borderRadius: 12, border: '1px solid var(--foundry-border-subtle)' }}>
              <p style={{ color: 'var(--foundry-text)', fontSize: 16, margin: 0 }}>{c.debate}</p>
              <div style={{ marginTop: 14, display: 'grid', gap: 10, gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, margin: 0, lineHeight: 1.6 }}><strong style={{ color: 'var(--foundry-text-faint)', fontWeight: 400 }}>Camp A: </strong>{c.campA}</p>
                <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, margin: 0, lineHeight: 1.6 }}><strong style={{ color: 'var(--foundry-text-faint)', fontWeight: 400 }}>Camp B: </strong>{c.campB}</p>
              </div>
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 14, fontStyle: 'italic' }}>{c.whyItEndures}</p>
            </article>
          ))}
        </div>
      )}
      ...
      {section === 'secrets' && (
        <div style={{ marginTop: 20, display: 'grid', gap: 12 }}>
          {lore.secrets.map((s) => (
            <article key={s.id} style={{ padding: 20, background: 'linear-gradient(135deg, var(--foundry-surface) 0%, #141418 100%)', borderRadius: 10, border: `1px solid ${accent}33` }}>
              <p style={{ color: accent, fontSize: 11, margin: 0, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{LORE_VOICE.secrets.label}</p>
              <p style={{ color: 'var(--foundry-text)', fontSize: 16, marginTop: 10, lineHeight: 1.5 }}>{s.headline}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 10, lineHeight: 1.65 }}>{s.body}</p>
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 12, marginTop: 12 }}>{s.whyFeelsSecret}</p>
            </article>
          ))}
        </div>
      )}
        ...
        <div style={{ marginTop: 24, borderLeft: `2px solid ${accent}44`, paddingLeft: 24 }}>
          {lore.timeline.map((e, i) => (
            <article key={`${e.year}-${i}`} style={{ marginBottom: 28, position: 'relative' }}>
              ...
              <p style={{ color: 'var(--foundry-text)', fontSize: 16, marginTop: 4 }}>{e.title}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 8, lineHeight: 1.65 }}>{e.body}</p>
            </article>
          ))}
        </div>
      )}
      ...
      {section === 'why-matters' && (
        <div style={{ marginTop: 20, display: 'grid', gap: 16 }}>
          {lore.whyMatters.map((w) => (
            <article key={w.topic} style={{ padding: 22, background: 'var(--foundry-surface-raised)', borderRadius: 12, border: '1px solid var(--foundry-border-subtle)' }}>
              <p style={{ color: accent, fontSize: 11, margin: 0, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{w.topic}</p>
              <p style={{ color: 'var(--foundry-text-faint)', fontSize: 14, marginTop: 12, textDecoration: 'line-through', lineHeight: 1.5 }}>{w.notThis}</p>
              <p style={{ color: 'var(--foundry-text)', fontSize: 17, marginTop: 12, lineHeight: 1.5, fontWeight: 400 }}>{w.insteadThis}</p>
              <p style={{ color: 'var(--foundry-text-muted)', fontSize: 14, marginTop: 12, lineHeight: 1.65 }}>{w.body}</p>
            </article>
          ))}
        </div>
      )}
        ...
        <div style={{ marginTop: 20 }}>
          ...
          <div style={{ marginTop: 20, display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
            {lore.experienceBeyond.map((f) => (
              <article key={f.id} style={{ padding: 18, background: 'var(--foundry-surface)', borderRadius: 10, border: '1px solid var(--foundry-border-subtle)' }}>
                <p style={{ color: 'var(--foundry-text)', fontSize: 15, margin: 0 }}>{f.factor}</p>
                <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 10, lineHeight: 1.6 }}>{f.why}</p>
...
function LoreField({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <>
      <dt style={{ color: 'var(--foundry-text-faint)', fontSize: 11, marginTop: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</dt>
      <dd style={{ color: accent ? accent : 'var(--foundry-text-muted)', margin: '4px 0 0', fontSize: 13, lineHeight: 1.6 }}>{value}</dd>
    </>
  );
}
...
function MysteryCard({ mystery, accent }: { mystery: { id: string; question: string; tease: string; answer: string; rabbitHoleHref?: string }; accent: string }) {
  ...
  return (
    <article style={{ padding: 20, background: 'var(--foundry-surface-raised)', borderRadius: 10, border: '1px solid var(--foundry-border-subtle)' }}>
      <p style={{ color: 'var(--foundry-text)', fontSize: 16, margin: 0 }}>{mystery.question}</p>
      <p style={{ color: 'var(--foundry-text-muted)', fontSize: 13, marginTop: 8 }}>{mystery.tease}</p>
      <button
        type="button"
        ...
        style={{
          ...
          background: open ? 'var(--foundry-border-warm)' : 'transparent',
          color: accent,
          cursor: 'pointer',
        }}
      >
      ...
      {open && (
        <div style={{ marginTop: 14 }}>
          <p style={{ color: 'var(--foundry-text)', fontSize: 14, lineHeight: 1.65 }}>{mystery.answer}</p>
