...
export function syncEnvelope(session: WorkshopSession): WorkshopSession {
  ...
  if (progress.multiply === 'complete' && progress.lead === 'locked') {
    progress.lead = 'open';
  }
  if (progress.lead === 'complete' && progress.build === 'locked') {
    progress.build = 'open';
  }
  if (progress.build === 'complete' && progress.earn === 'locked') {
    progress.earn = 'open';
  }
  if (progress.earn === 'complete' && progress.own === 'locked') {
    progress.own = 'open';
  }
