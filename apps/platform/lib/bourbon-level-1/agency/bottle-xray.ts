...
const DEFAULT_WAREHOUSE = 'Rickhouse aging — floor position affects extraction rate and angel\'s share.';
const DEFAULT_ENTRY = 'Barrel entry proof typically 110–125 — lower entry often yields softer aged whiskey.';
...
function buildXRay(b: BourbonBottle): BottleXRay {
  ...
  return {
    ...
    layers: [
      ...
      { label: 'Barrel entry proof', value: DEFAULT_ENTRY, whyItMatters: 'Entry proof sets aging trajectory — same mash, different entry, different outcome.' },
      { label: 'Warehouse', value: DEFAULT_WAREHOUSE, whyItMatters: 'Top floors age faster and hotter; single barrels expose this variance.' },
    ...
    flavorBreakdown,
    analystTake: `${b.name}: ${b.oneLiner} ${b.whyBuy}`,
  };
}
